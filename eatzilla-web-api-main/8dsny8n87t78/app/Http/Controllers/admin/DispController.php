...
class DispController extends BaseController
{
     ...
     */
    public function edit_dispmanager(Request $request)
    {
        $access_detail = $this->access_privilages->where('admin_id',$request->id)->first();
        $admin_detail = $this->admin->where('id',$request->id)->first();
        return view('edit_dispmanager',compact('access_detail','admin_detail'));
    }
