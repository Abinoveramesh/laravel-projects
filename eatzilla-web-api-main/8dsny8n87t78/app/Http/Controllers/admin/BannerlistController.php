...
class BannerlistController extends BaseController
{
	public function banner_list(Request $request)
	{
		...
		$data = DB::table('offers_banner')
				...
				->select('restaurants.restaurant_name as restaurant_name','offers_banner.image1 as banner_image','offers_banner.position as banner_position','offers_banner.status as banner_status','offers_banner.is_suffle as banner_suffle','offers_banner.id as banner_id')
				->get();

				foreach($data as $d)
				{
					$d->banner_image = SPACES_BASE_URL.$d->banner_image;
				}
	...
	public function add_banner(Request $request)
	{
		...
		$data = $restaurant_list->where('parent',0)->get();
	...
	public function add_to_banners(Request $request)
	{
		if($request->id)
        {
			$validator = Validator::make($request->all(), [
                ...
                'status' => 'required',
    			'new_banner_image' => 'mimes:jpeg,jpg,bmp,png'
            ]);
		}else
		{
			$validator = Validator::make($request->all(), [
                'restaurant_id' => 'required',
                'status' => 'required',
    			'new_banner_image' => 'required|mimes:jpeg,jpg,bmp,png'
            ]);
		}
        if($validator->fails()) {
        ...
        }else
        {
        	...
        	{
        		$image1 = $custom->upload_image($request,'new_banner_image');
			 	$image1 = $image1;
        	}
        	if($request->id)
        	{
	...
	{
		$restaurant_list = $this->restaurants;
		$restaurants = $restaurant_list->where('parent',0)->get();

		$banner = $this->banner;
		$data = $banner::where('id',$banner_id)->first();
		$data->image =  SPACES_BASE_URL .$data->image;
		$data->image1 =  SPACES_BASE_URL .$data->image1;
