...
class DashboardController extends BaseController
{
   ...
   //for super admin dashboard //
    ...
    $role = $request->session()->get('role');
		...
		$total_admin_comission = DB::table('requests')->where('status',7)->sum('admin_commision');

		$total_restaurant_comission = DB::table('requests')->where('status',7)->sum('restaurant_commision');

		$total_delivery_boy_comission = DB::table('requests')->where('status',7)->sum('delivery_boy_commision');
