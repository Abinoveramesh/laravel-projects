...
class ItemmasterController extends BaseController
{
	...
	public function index()
	{
		...
		return view('add_category',['restaurants'=>$restaurants]);
	}
	...
	public function get_category_list(Request $request)
	{
		...
		$data = DB::table('category')->where('category.status','!=',0)
									 ->leftjoin('restaurants','restaurants.id','=','category.restaurant_id')
									 ->select('category.id as cat_id','category.*','restaurants.*')
									 ->get();
	...
	{
		// $category_list = $this->category;
		$data = DB::table('category')->where('category.id',$category_id)
											  ->leftjoin('restaurants','restaurants.id','=','category.restaurant_id')
											  ->select('category.id as cat_id','category.*','restaurants.*')
											  ->first();
		...
		return view('add_category',['data'=>$data, 'restaurants'=>$restaurants]);
	}
		...
		public function add_to_cuisines(Request $request)
	{
        ...
        }else
        {
			...
			if($request->id)
			{
				$this->cuisines->where('id',$request->id)->update(['name'=>$cuisine_name,'cuisine_image'=>$image]);
			}else
			{
					$data = array();
	 ...
	 */
	public function add_cuisine_ajax(Request $request)
	{
		$validator = Validator::make($request->all(), [
			'cuisine_name' => 'required|max:30',
		]);
		...
		}else
		{
			...
			return response()->json(['status'=>true,'details'=> $this->cuisines ]);
		}
	}
}