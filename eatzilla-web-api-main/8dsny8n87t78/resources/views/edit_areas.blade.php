 ...
 <div class="content-wrapper">
      <div class="content-header row">
        <div class="content-header-left col-md-6 col-12 mb-2 breadcrumb-new">
          <h3 class="content-header-title mb-0 d-inline-block">{{ strtoUpper(trans('constants.edit')) }} {{ strtoUpper(trans('constants.area')) }}</h3>
          <div class="row breadcrumbs-top d-inline-block">
            <div class="breadcrumb-wrapper col-12">
              <ol class="breadcrumb">
              <!--   <li class="breadcrumb-item"><a href=" "></a>
                </li> -->
                <li class="breadcrumb-item"><a href="{{url('/')}}/admin/view_areas/{{$area->add_city_id}}">{{ strtoUpper(trans('constants.area')) }} {{ strtoUpper(trans('constants.list')) }}</a>
                </li>
                <li class="breadcrumb-item"><a href=" ">{{ strtoUpper(trans('constants.edit')) }} {{ strtoUpper(trans('constants.area')) }}</a>
                </li>
              </ol>
            </div>
          </div>
            ...
            <div class="col-12">
              <div class="card">
                ...
                <div class="card-content collapse show">
                  <div class="card-body">
                    <form action="{{url('/')}}/admin/update_area_list" method="post"   class="icons-tab-steps wizard-notification">
                        ...
                        <div class="row">
                          ...
                          <!-- <div class="col-md-6">
                            <div class="form-group">
                              ...
                              <input type="text" class="form-control" disabled="" value="{{$area->area}}"  id="eventName2" placeholder="">
                            </div>
                          </div> -->
                          ...
                          <div class="col-md-6">
                            <div class="form-group">
                             ...
                             <input type="text" value="@if(old('area')) {{ old('area')}} @else {{$area->area}} @endif" required class="form-control" name="area" id="area" placeholder="Area">
                             @if ($errors->has('area'))
                          ...
                          <div class="col-md-6">
                            <div class="form-group">
                              ...
                              <select name="status" id="" class="form-control" required="">
                                ...
                                <option value="1" @if(old('status')) @if( old('status') ==2) {{ 'selected' }} @endif @else @if( $area->status ==2) {{ 'selected' }} @endif @endif>{{trans('constants.active')}}</option>
                                <option value="2" @if(old('status')) @if( old('status') ==2) {{ 'selected' }} @endif @else @if( $area->status ==2) {{ 'selected' }} @endif @endif>{{trans('constants.inactive')}}</option>
                              </select> 
                          ...
                          <div class="col-md-12">
                            <div class="form-group">
                              ...
                              <textarea id="geofence_latlng" name="geofence_latlng" style="display:none;">@if(old('geofence_latlng')) {{old('geofence_latlng')}}
                                @else {{$area->polygons}}
                                @endif</textarea>
                                <input type="hidden" id="latitude" name="latitude" value="@if(old('latitude')) {{ old('latitude') }} @else {{$area->latitude}} @endif">
                                <input type="hidden" id="longitude" name="longitude" value="@if(old('longitude')) {{ old('longitude') }} @else {{$area->longitude}} @endif">
