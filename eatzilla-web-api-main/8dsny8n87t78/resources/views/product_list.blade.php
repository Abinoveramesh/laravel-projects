...
@endphp
  <div class="content-wrapper">
                ...
                <div class="card-head">
                  ...
                  <div class="heading-elements">
                     <ul class="list-inline mb-0">
                      ...
                      <li @if(isset(auth()->user()->role) && auth()->user()->role==3 && !in_array(2,(array)$foodAccess)) style="display:none" @endif> <button class="btn btn-primary btn-sm"><i class="ft-plus white"></i><a style=" color: white !important;" href="{{URL('/')}}/admin/choice_category"> {{trans('constants.add')}} {{trans('constants.product')}}</a></button></li>
                      </ul>
                      </div>
               ...
               <div class="card-content collapse show">
                  <div class="card-body card-dashboard">
                  	@if(session()->get('role')==1)
			           <div class="row">
			           		<div class="col-md-2">
			           		   <label style="line-height: 38px;">Restaurant Name : </label>
			           		 </div>
			           		 <div class="col-md-4">
			           		   <select class="c-select form-control" id="restaurant_id" name="restaurant_id">
				               		@foreach($restaurant_list as $r)
					                	<option value="{{$r->id}}" @if($r->id == session()->get('restaurant_id')) selected="" @endif>{{$r->restaurant_name}}</option>
					                @endforeach
				               </select>
				            </div>
				            <div class="col-md-6">
				            	<button class="form-group btn btn-info" style="padding:9px;" onclick="restaurant_filter()">Filter</button>
				            </div>
			       	   </div>
		            @endif
                      ...
                      <thead> 
                        ...
                        <tbody>
                          ...
                          <tr>
                            ...
                            <td>@isset($d->Category) @foreach($d->Category as $c) {{$c->category_name}} <br> @endforeach @endisset</td>
                            <!-- <td>{{$d->tax}}</td> -->
                            ...
                            @endif
                            <td>
                               <li @if(isset(auth()->user()->role) && auth()->user()->role==3 && !in_array(3,(array)$foodAccess)) style="display:none" @endif><a href="{{url('/')}}/admin/edit_choice_category/{{$d->id}}" class="button btn btn-icon btn-success mr-1 link_clr"><i class="ft-edit"></i></a>
  	...
  	<script>
  		function restaurant_filter(){
  			var restaurant_id = $('#restaurant_id').val();
  			if(restaurant_id != ''){
  				window.location = '{{url('/')}}/admin/product_list/'+restaurant_id;
  			}else{
  				window.location = '{{url('/')}}/admin/product_list';
  			}
