          ...
          <div class="row breadcrumbs-top d-inline-block">
            <div class="breadcrumb-wrapper col-12">
              <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="#" class="brand-font-link-color">{{ strtoUpper(trans('constants.google_setting')) }}</a>
                </li>
              </ol>
            </div>
          </div>
                    ...
                    <form action="{{url('/')}}/admin/update-setting" class="icons-tab-steps wizard-notification" method="post" enctype="multipart/form-data">
                     ...
                     <fieldset>
                      <div class="row">
                       ...
                       @endif
                       @if(isset($data['android_api_key']))
                         <div class="form-group row">                        
                           <label class="col-md-2">{{ trans('constants.android_api_key') }} </label>                          
                           <div class="col-md-10">
                               <input type="text" name="android_api_key" class="form-control" placeholder="Title of the message" required="" value="@if(isset($data['android_api_key'])){{$data['android_api_key']}}@endif">
                          </div>
                       </div>
                       @endif
                       @if(isset($data['ios_api_key']))
                         <div class="form-group row">                        
                           <label class="col-md-2">{{ trans('constants.ios_api_key') }} </label>                          
                           <div class="col-md-10">
                               <input type="text" name="ios_api_key" class="form-control" placeholder="Title of the message" required="" value="@if(isset($data['ios_api_key'])){{$data['ios_api_key']}}@endif">
                          </div>
                       </div>
                       @endif
